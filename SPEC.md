...
4. **Cumulative billed on a PO** = sum of totals of previously APPROVEd invoices matched to that PO. REVIEW/HOLD items don't count until resolved as approved.
5. **The process never hard-rejects.** Negative final calls are human decisions made on the dashboard.
6. **Duplicate** = same normalized invoice number + same vendor as any previously processed run (any outcome except HOLD/NOT_AN_INVOICE).
7. Ingestion is simulated locally (bundled inbox). Production adapters (IMAP) share the same interface.

## 2. Decision object (the output)

```ts
type Outcome = "APPROVE" | "REVIEW" | "HOLD";

interface Decision {
  outcome: Outcome;
  priority: "normal" | "high";        // high = >10% over, fraud, multiple flags
...
Critical fields: `vendor_name`, `invoice_number`, `total`. Missing → HOLD. `invoice_date` missing → REVIEW.
...
Severity aggregation: **HOLD > REVIEW > APPROVE.** `security=true` if any fraud rule fires.
`priority=high` if AMOUNT_SIGNIFICANTLY_OVER, PO_OVERBILLED, any fraud rule, or ≥3 reasons.

### Data quality (→ HOLD)

| Code | Trigger | Message template |
|---|---|---|
| NOT_AN_INVOICE | `is_invoice=false` | "This document doesn't appear to be an invoice." |
| UNREADABLE_DOCUMENT | extraction failed after 1 retry | "Couldn't read this document. It may be corrupted or too low quality." |
| MISSING_CRITICAL_FIELD | vendor_name, invoice_number or total is null | "Critical information is missing: {fields}. Can't evaluate without it." |
| MATH_INCONSISTENT | \|subtotal + tax − discount − total\| > 0.02, or \|Σline_items − subtotal\| > 0.02 (only when itemised) | "The numbers don't add up: {detail}." |
| LOW_CONFIDENCE_TOTAL | field_confidence.total = "low" | "Couldn't read the total amount reliably." |
...
| CURRENCY_MISMATCH | invoice currency ≠ PO currency | "Invoice is in {inv_ccy} but the PO is in {po_ccy} — amounts can't be compared directly." |
...
| AMOUNT_OVER_TOLERANCE | this invoice alone: 5% < overage ≤ 10% vs PO remaining | "Invoice is {pct}% over the PO — beyond the 5% tolerance." |
| AMOUNT_SIGNIFICANTLY_OVER | overage > 10% | "Invoice is {pct}% over the PO — well beyond tolerance. High priority." |
| PO_OVERBILLED | (approved-to-date + this invoice) > PO total × 1.05 | "With this invoice, PO {po} would be billed {cum} of {po_total} ({pct}%) — over-billed across {n} invoices." |
...
cumulative check above. Tolerance base: 5%.
...
### Fraud / security (→ REVIEW, security=true, priority=high)
...
| INV_004_amount_mismatch_globaltech.pdf | ~10.5% over | REVIEW (high) |
| INV_005_amount_mismatch_prime.pdf | ~5.5% over | REVIEW |
| INV_006_duplicate_of_001.pdf | duplicate (run after 001) | REVIEW — DUPLICATE |
| INV_007_unknown_vendor.pdf | unknown vendor, no PO | REVIEW |
| INV_008_bank_change_globaltech.pdf | BEC bank change | REVIEW 🛡️ |
| INV_009_no_po_riverside.pdf | inactive vendor, no PO | REVIEW |
| INV_011_scanned_style_prime.pdf | clean | APPROVE |
| INV_012_multicurrency_eur.pdf | EUR vs USD PO | REVIEW — CURRENCY_MISMATCH |
| INV_013_suspicious_round_amount.pdf | unknown + round 50k + no detail | REVIEW 🛡️ (high) |
...
| GEN_split_metro_3.pdf ($1,560) | cumulative $3,960 > $3,780 (105%) | REVIEW — PO_OVERBILLED (high) |
| GEN_fraud_globaltech.pdf (bank change + URGENT + lookalike domain) | BEC composite | REVIEW 🛡️ (high) |
| GEN_missing_fields.pdf (no invoice #, no total) | missing critical data | HOLD |
| GEN_not_invoice.pdf (a letter) | not an invoice | HOLD — NOT_AN_INVOICE |
...
2. INV_006 duplicate → REVIEW with named original (control)
3. GEN_split_metro_3 → REVIEW PO_OVERBILLED (stateful matching)
4. GEN_fraud_globaltech → REVIEW 🛡️ high (fraud module)

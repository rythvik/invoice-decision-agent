// Human action on a FINISHED run (never mid-run): approve or reject a REVIEW/HOLD item.
import { ensureSeeded } from "@/lib/bootstrap";
import { getRun, resolveRun } from "@/lib/db";
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (run.outcome === "APPROVE") return Response.json({ error: "already auto-approved" }, { status: 400 });
  ...
  resolveRun(id, resolution, String(body.note).trim());
